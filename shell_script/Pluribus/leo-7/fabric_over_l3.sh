...
cli fabric-comm-vrouter-bgp-create name $vrouter bgp-as $bgp_as router-id $router_id bgp-redistribute connected bgp-max-paths 16 in-band-nic-ip $in_band_ip bgp-nic-ip $bgp_ip bgp-nic-l3-port $port remote-as $remote_as neighbor $neighbor_ip fabric-network $fabric_network bfd
