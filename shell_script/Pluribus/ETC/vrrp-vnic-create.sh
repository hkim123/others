...
##	cli --quiet vlan-create id $vlan scope fabric >> $log
	...
	output=`cli --quiet vrouter-interface-add vrouter-name $vrname ip 22.$ip1.$ip2.$primaryip/24 vlan $vlan`
	if [ $rc -ne 0 ]
	then
	...
	cli --quiet vrouter-interface-add vrouter-name $vrname ip 22.$ip1.$ip2.1/24 vlan $vlan vrrp-id $vrrpid vrrp-primary $primarynic >> $log
	rc=$?
	if [ $rc -ne 0 ]
	then
